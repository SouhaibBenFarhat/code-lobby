...
const electronAPI: ElectronAPI = {
  ...
  generateDailySpeechStreaming: (options: {
    ...
    githubToken: string
  }) => ipcRenderer.invoke('generate-daily-speech-streaming', options),

  stopDailySpeech: (sessionId: string) => ipcRenderer.invoke('stop-daily-speech', sessionId),
  ...
  onDailySpeechChunk: (
    ...
    ipcRenderer.on('daily-speech:chunk', handler)
    return () => {
      ipcRenderer.removeListener('daily-speech:chunk', handler)
    }
  },
  ...
  onDailySpeechDone: (
    ...
    ipcRenderer.on('daily-speech:done', handler)
    return () => {
      ipcRenderer.removeListener('daily-speech:done', handler)
    }
  },

  onDailySpeechError: (callback: (data: { sessionId: string; error: string }) => void) => {
    const handler = (
      _event: Electron.IpcRendererEvent,
    ...
    ipcRenderer.on('daily-speech:error', handler)
    return () => {
      ipcRenderer.removeListener('daily-speech:error', handler)
    }
  },
  ...
  db: {
    ...
    // Daily Reports
    dailyReports: {
      list: () => ipcRenderer.invoke('db:dailyReports:list'),
      get: (id: string) => ipcRenderer.invoke('db:dailyReports:get', id),
      getByDate: (date: string) => ipcRenderer.invoke('db:dailyReports:getByDate', date),
      create: (data: {
        id: string
        date: string
        content: string
        ...
        thinking?: string | null
      }) => ipcRenderer.invoke('db:dailyReports:create', data),
      upsert: (data: {
        id: string
        date: string
        content: string
        ...
        toolsUsed?: string | null
        thinking?: string | null
      }) => ipcRenderer.invoke('db:dailyReports:upsert', data),
      update: (
        id: string,
        data: {
          content?: string
      ...
      ) => ipcRenderer.invoke('db:dailyReports:update', id, data),
      delete: (id: string) => ipcRenderer.invoke('db:dailyReports:delete', id),
      listRecent: (limit?: number) => ipcRenderer.invoke('db:dailyReports:listRecent', limit)
    },
