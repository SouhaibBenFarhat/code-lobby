...
export function buildPRSystemPrompt(pr: PullRequest, changedFiles?: ChangedFile[]): string {
  ...
  lines.push('')
  lines.push(
    'When reviewing code and finding issues, format EACH finding as a separate block so users can post them individually to the PR.'
  )
  lines.push('')
  lines.push('### Required Format for Postable Findings')
  lines.push('')
  lines.push('For EACH bug, issue, or suggestion at a specific location, use this EXACT format:')
  lines.push('')
  lines.push('```')
  lines.push('---')
  ...
  lines.push('')
  lines.push('Description of the issue and why it matters.')
  lines.push('')
  ...
  lines.push('')
  lines.push('### Example with Multiple Findings')
  lines.push('')
  lines.push('```')
  ...
  lines.push('')
  lines.push('Accessing `user.profile.name` without null check will crash if user has no profile.')
  lines.push('')
  lines.push('**Suggestion:** Use optional chaining: `user?.profile?.name`')
  lines.push('<!--POSTABLE:{"file":"src/utils/auth.ts","line":42}-->')
  lines.push('')
  lines.push('---')
  lines.push('### 🟠 Warning: Missing Error Handling')
  lines.push('**File:** `src/api/handler.ts` **Line:** 15')
  lines.push('')
  lines.push('This async call has no try-catch, errors will be swallowed.')
  lines.push('')
  lines.push('**Suggestion:** Wrap in try-catch and log errors.')
  lines.push('<!--POSTABLE:{"file":"src/api/handler.ts","line":15}-->')
  lines.push('')
  lines.push('---')
  lines.push('**Summary:** Found 2 issues - 1 critical, 1 warning.')
  lines.push('```')
  lines.push('')
  lines.push('### Rules')
  lines.push('- Put `<!--POSTABLE:...-->` IMMEDIATELY after each finding (not at the end)')
  ...
  lines.push('- End with a brief summary if multiple findings')
  lines.push('')
  lines.push('### When NOT to Use POSTABLE')
  lines.push('- General explanations or summaries (no specific line)')
