...
export function buildPRSystemPrompt(pr: PullRequest, changedFiles?: ChangedFile[]): string {
  ...
  lines.push('```')
  lines.push('')
  lines.push('> **PR Comment:** Brief explanation + code snippet showing the fix')
  lines.push('> ```language')
  lines.push('> // corrected code here')
  lines.push('> ```')
  ...
  lines.push(
    '**IMPORTANT:** Always include a code snippet showing the correct code in the PR Comment.'
  )
  lines.push('The reviewer should see exactly how to fix the issue without guessing.')
  lines.push('')
  ...
  lines.push('```')
  lines.push('')
  lines.push('> **PR Comment:** Potential null pointer exception. Use optional chaining:')
  lines.push('> ```ts')
  lines.push('> const name = user?.profile?.name ?? "Unknown";')
  lines.push('> ```')
  ...
  lines.push('```')
  lines.push('')
  lines.push('> **PR Comment:** Unhandled promise rejection. Wrap in try-catch:')
  lines.push('> ```ts')
  lines.push('> try {')
  lines.push('>   await fetchData(url);')
  lines.push('> } catch (error) {')
  ...
  lines.push('')
  lines.push('### Rules')
  lines.push('- Always show **Current code** and **Fix** code snippets in your analysis')
  lines.push('- **PR Comment MUST include a code snippet** showing the correct code')
