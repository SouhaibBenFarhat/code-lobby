...
import { Button, Col, cn, Row, ScrollArea, Separator, ViewHeader } from '@codelobby/ui-kit'
import {
  Bot,
  CheckCircle2,
  FileCode,
...
export function PRDetail({ onClose }: PRDetailProps): React.JSX.Element | null {
  ...
  // Show loading state when fetching full details
  const showLoadingState = isLoadingDetails || !pr
  ...
  return (
    <div
      ...
      style={{ maxWidth: '100%' }}
    >
      {/* Show loading header when PR data not yet loaded, full header when loaded */}
      {showLoadingState ? (
        <ViewHeader
          icon={<GitPullRequest className="w-4 h-4" />}
          title={`PR #${selectedPRId.prNumber}`}
          subtitle={selectedPRId.repoFullName}
          rightContent={
            <Button
              variant="ghost"
              size="icon"
      ...
      {/* Loading state while fetching full PR details */}
      {showLoadingState ? (
        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
          <Loader2 className="w-8 h-8 animate-spin text-primary mb-3" />
          <span className="text-sm font-medium">Loading PR details...</span>
          <span className="text-xs mt-1">Fetching comments, reviews, and more</span>
        </div>
      ) : (
